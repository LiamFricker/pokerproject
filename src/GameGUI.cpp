...
void hide_buttons(){
    ...
    //printf("SUFFERING BUTTONS HIDDEN\n");
}

void set_label(std::string setlabel){
    current_main = setlabel;
    setlabel = "<span size=\"large\">Name: " + current_main + "Time: " + current_time + "</span>";
    gtk_label_set_markup(GTK_LABEL (label_main), setlabel.c_str());
}


void set_time(int clocktime){
    //std::cout << "clocktime" << clocktime << std::endl;
    current_time = std::to_string(clocktime);
    std::string setlabel = "<span size=\"large\">Name: " + current_main +"Time: " + current_time + "</span>";
    gtk_label_set_markup(GTK_LABEL (label_main), setlabel.c_str());
}
...
{
    printf ("Raise was pressed. Raise amount: %i\n", (int) gtk_range_get_value(data));
    //raise(player, (int) gtk_range_get_value(data));
    sendBet(2, (int) gtk_range_get_value(data));
    hide_buttons();
}
...
void GameGUIInit()
{
    ...
    */
    ...
    gtk_widget_set_size_request (GTK_WIDGET (window1), 100, 100); //1030,900 without buttons
